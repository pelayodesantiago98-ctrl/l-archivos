 ...
 * alguien -- asi que quien lo reciba puede reenviarlo. Por eso caduca solo, por
 * eso se puede revocar a mano, y por eso la pantalla lo dice con todas las
 * letras antes de crear ninguno.
 *
...
function limpiar(datos) {
  ...
  for (const [token, e] of Object.entries(datos)) {
    if (!e || !e.caduca || e.caduca < ahora) { delete datos[token]; fuera++; }
  }
  return fuera;
}
...
function crear(tipo, rel, dias) {
  ...
  datos[token] = {
    ...
    creado: Date.now(),
    caduca: Date.now() + cuantos * 86400000,
    vistas: 0,
  };
  escribir(datos);
...
function resolver(token, contar) {
  ...
  const datos = leer();
  const e = datos[token];
  if (!e || !e.caduca || e.caduca < Date.now()) return null;
...
function listar() {
  ...
  return Object.entries(datos)
    .map(([token, e]) => ({
      ...
      dias: Math.max(0, Math.ceil((e.caduca - Date.now()) / 86400000)),
    }))
