...
function crear(usuario, tipo, rel, dias) {
  /* Uno o varios: se acepta un nombre suelto o una lista, y por dentro
     siempre es una lista. */
  const lista = (Array.isArray(rel) ? rel : [rel])
  ...
  for (const uno of lista) {
    const abs = ficheros.resolver(usuario, tipo, uno);
    if (!abs || !fs.statSync(abs).isFile()) {
      const err = new Error('«' + path.basename(uno) + '» no existe o no es un fichero.');
      err.status = 404;
      throw err;
    }
  }
  ...
  const nombre = lista.length === 1
    ? path.basename(lista[0])
    : lista.length + ' archivos';
  ...
  const huella = lista.slice().sort().join('\u0000');
  for (const [token, e] of Object.entries(datos)) {
    if (e.usuario !== usuario || e.tipo !== tipo) continue;
    const suyas = (e.rels || [e.rel]).slice().sort().join('\u0000');
    if (suyas === huella) {
      return { token, caduca: e.caduca, nombre: e.nombre || nombre,
               cuantos: lista.length, reutilizado: true };
    }
  }
  ...
  escribir(datos);
  return { token, caduca: datos[token].caduca, nombre, cuantos: lista.length };
}
...
function resolver(token, contar) {
  ...
  /* Las entradas de antes traen `rel` a secas. Se leen como una lista de uno
     sin tocar el almacen: reescribirlo entero por esto seria arriesgar los
     enlaces que ya estan repartidos. */
  const lista = e.rels || (e.rel ? [e.rel] : []);

  const cosas = [];
  for (const uno of lista) {
    const abs = ficheros.resolver(e.usuario, e.tipo, uno);
    ...
    if (abs) cosas.push({ rel: uno, nombre: path.basename(uno), abs });
  }
