...
function listar() {
  const salida = [];
  // Una sola lectura del almacen para todo el listado, no una por fichero
  const marcadas = favoritos.todas();
  for (const [tipo, raiz] of Object.entries(CARPETAS)) {
    for (const f of recorrer(raiz)) {
      salida.push({
        ...
        fecha: f.fecha,
        favorito: marcadas.has(tipo + '/' + f.rel),
      });
    }
  }
...
function borrar(tipo, rel) {
  /*
   * Una favorita no se borra, y se comprueba AQUI y no solo en la pantalla.
   *
   * Ocultar el boton evita el despiste, que es el 99% de los casos, pero la
   * peticion se puede lanzar igual desde fuera de la pagina -- una pestana
   * vieja con el listado de antes, por ejemplo, donde esa pelicula todavia
   * salia con su boton. Con 3 GB de pelicula y sin papelera, el sitio donde
   * tiene que estar la negativa es este.
   */
  if (favoritos.es(tipo, rel)) {
    const err = new Error('Esa esta en favoritos: quitala de favoritos antes de borrarla.');
    err.status = 409;
    throw err;
  }
